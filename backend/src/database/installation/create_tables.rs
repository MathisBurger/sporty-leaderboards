...
pub async fn create_workouts_table(service: &database_service::DatabaseService) -> bool {
    query("CREATE TABLE `sporty-leaderboards`.`workouts` ( `id` INT NOT NULL AUTO_INCREMENT , `username` TEXT NOT NULL , `time` INT NOT NULL , `distance` INT NOT NULL , `timestamp` BIGINT UNSIGNED NOT NULL , PRIMARY KEY (`id`)) ")
        .execute(&service.conn).await.is_ok()
}